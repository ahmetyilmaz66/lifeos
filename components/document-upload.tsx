...
export default function DocumentUpload() {
  ...
  async function uploadFile() {
    ...
    const storagePath = `${userId}/${documentId}/${sanitizeFileName(file.name)}`;
    ...
    const { error: insertError } = await supabase.from("documents").insert({ id: documentId, user_id: userId, file_name: sanitizeFileName(file.name), file_type: file.type, storage_path: storagePath, file_size: file.size, processing_status: "uploaded" });
    if (insertError) {
    ...
    setFile(null); setMessage("Belgen LifeOS'a eklendi. Analiz için hazır."); setUploading(false); router.refresh();
  }

  return <div className="space-y-5 rounded-xl border border-border bg-card p-6 sm:p-8">
    <div><h2 className="text-lg font-semibold">Belge yükle</h2><p className="mt-1 text-sm text-muted-foreground">PDF, JPG, JPEG, PNG veya WEBP · En fazla 10 MB</p></div>
    ...
    <Button type="button" disabled={!file || uploading} onClick={uploadFile}>{uploading ? <Loader2 className="animate-spin" size={16} /> : <FileUp size={16} />}{uploading ? "Yükleniyor..." : "Belgeyi yükle"}</Button>
  </div>;
}